...
void GxEPD2_420c_NM_UC8179::clearScreen(uint8_t value) { clearScreen(value, 0xFF); }
...
void GxEPD2_420c_NM_UC8179::writeScreenBuffer(uint8_t value) { writeScreenBuffer(value, 0xFF); }
...
void GxEPD2_420c_NM_UC8179::_writeScreenBuffer(uint8_t command, uint8_t value)
{
    ...
    for (uint32_t i = 0; i < uint32_t(WIDTH) * uint32_t(HEIGHT) / 8; i++) _transfer(tx);
    _endTransfer();
}
...
{
    _writeImagePart(0x10, bitmap, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h,
                    invert, mirror_y, pgm);
}
...
void GxEPD2_420c_NM_UC8179::writeImagePart(const uint8_t* black, const uint8_t* color,
        ...
        int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
    if (black) _writeImagePart(0x10, black, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h,
                               invert, mirror_y, pgm);
    if (color) _writeImagePart(0x13, color, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h,
                               invert, mirror_y, pgm);
}
...
void GxEPD2_420c_NM_UC8179::powerOff() { _PowerOff(); }
