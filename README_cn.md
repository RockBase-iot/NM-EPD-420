...
* **显示屏**：4.2" 400×300 三色电子墨水屏（黑 / 白 / 红），型号 **GDEY042Z98**；黑白墨水屏（GxEPD2_420_GYE042A87）；以及四色屏（黑 / 白 / 红 / 黄，GxEPD2_420c_GDEY0420F51）。三者使用相同引脚定义，但控制器协议必须匹配，根据实际使用屏幕来选择相应变体。
...
当前NM-EPD-420支持三色墨水屏（GDEY042Z98），黑白墨水屏（GYE042A87）以及四色墨水屏（GDEY0420F51），三者的刷新性能如下：
...
- **GDEY0420F51 四色墨水屏**：
  ...
  - 对于需要显示更多颜色，刷新又不频繁的应用，建议使用四色墨水。
  - **构建环境： `nm-epd-420-4c`**，驱动为 `GxEPD2_420c_GDEY0420F51`（400x300，黑/白/红/黄）。
  - 使用 `pio run -e nm-epd-420-4c` 构建；T1 依次检查白、黑、红、黄全屏和黑/红/黄文字。

使用NM-EPD-420底板对7.5寸三色屏进行了测试（GxEPD2_750c_Z08，控制器GD7965），验证了底板的兼容性和驱动的可行性，测试结果表明可以正常刷新显示内容。
*注意：7.5寸屏非NM-EPD-420配套标准屏，仅用于测试，当然应用需要可适当调整外壳、结构等以适配实际应用使用。*
...
![7.5寸三色屏测试](image/nm-epd-750-3c.png)
...
* **墨水屏**：推荐 `zinggjm/GxEPD2` 库，默认三色 NM-EPD-420 使用 `GxEPD2_420c_GDEY042Z98`，黑白 NM-EPD-420-BW 使用 `GxEPD2_420_GYE042A87`。四色面板可使用 `GxEPD2_420c_GDEY0420F51` 和 `nm-epd-420-4c` 环境。CS=46, DC=4, RST=5, BUSY=6；四色驱动的 BUSY 有效电平为低。
